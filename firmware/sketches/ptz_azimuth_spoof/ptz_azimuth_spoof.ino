...
const unsigned long FIRST_PULSE_MS = 200;
const unsigned long SECOND_PULSE_MS = 200;
const unsigned long THIRD_PULSE_MS = 450;
...
void setup() {
  ...
  pulseRelay(RELAY1_PIN, FIRST_PULSE_MS);

  waitUntilMs(SECOND_SPOOF_B_MS);
  pulseRelay(RELAY2_PIN, SECOND_PULSE_MS);

  waitUntilMs(THIRD_SPOOF_B_MS);
  pulseRelay(RELAY2_PIN, THIRD_PULSE_MS);
}
