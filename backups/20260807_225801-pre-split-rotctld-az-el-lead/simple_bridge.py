...
ROTCTLD_TRACK_LEAD_SECONDS = float(os.environ.get("ROTCTLD_TRACK_LEAD_SECONDS", "1.5"))
ROTCTLD_PARK_IGNORE_SEC = float(os.environ.get("ROTCTLD_PARK_IGNORE_SEC", "6.0"))
...
def set_rotor_net_target(az_deg, el_deg, client_name=""):
    ...
    if client_name == "ui-home":
        reset_rotctld_track_state(ROTCTLD_PARK_IGNORE_SEC)
...
def reset_rotctld_track_state(ignore_seconds=0.0):
    rotctld_client_streams.clear()
    rotctld_track_lead["sourceAz"] = None
    rotctld_track_lead["sourceEl"] = None
    rotctld_track_lead["sourceTs"] = 0.0
    rotctld_track_control["ignoreUntil"] = time.time() + max(0.0, float(ignore_seconds))
...
def set_rotor_net_streaming_target(az_deg, el_deg, client_name="rotctld-track"):
    ...
    antenna_el = clamp(float(el_deg), 0.0, 90.0)
    now = time.time()
    target_az = antenna_az
    target_el = antenna_el
    lead_seconds = max(0.0, float(ROTCTLD_TRACK_LEAD_SECONDS))
    prev_az = rotctld_track_lead.get("sourceAz")
    prev_el = rotctld_track_lead.get("sourceEl")
    prev_ts = float(rotctld_track_lead.get("sourceTs") or 0.0)
    dt = now - prev_ts if prev_ts > 0.0 else 0.0
    if lead_seconds > 0.0 and prev_az is not None and prev_el is not None and 0.05 <= dt <= ROTCTLD_TRACK_STALE_SEC:
        az_rate = normalize_angle_delta(antenna_az - float(prev_az)) / dt
        el_rate = (antenna_el - float(prev_el)) / dt
        az_rate = clamp(az_rate, -ROTOR_TRACK_LEAD_AZ_RATE_DEG_PER_SEC, ROTOR_TRACK_LEAD_AZ_RATE_DEG_PER_SEC)
        el_rate = clamp(el_rate, -ROTOR_TRACK_LEAD_EL_RATE_DEG_PER_SEC, ROTOR_TRACK_LEAD_EL_RATE_DEG_PER_SEC)
        target_az = norm_az(antenna_az + az_rate * lead_seconds)
        target_el = clamp(antenna_el + el_rate * lead_seconds, 0.0, 90.0)
    rotctld_track_lead["sourceAz"] = antenna_az
    rotctld_track_lead["sourceEl"] = antenna_el
    rotctld_track_lead["sourceTs"] = now
    with state_lock:
        ...
        if entering_stream:
            ...
            rotor_net["lastSendMs"] = 0.0
            rotor_net["lastCmd"] = "PTZ STOP"
        rotor_net["targetAz"] = target_az
        rotor_net["targetEl"] = target_el
        rotor_net["active"] = True
        rotor_net["busy"] = False
        rotor_net["lastClient"] = client_name
        rotor_net["lastUpdateMs"] = now
        latest["rotorSource"] = "NET"
        latest["status"] = (
            f"ROTCTLD TRACK LEAD AZ {antenna_az:.1f}->{target_az:.1f} "
            f"EL {antenna_el:.1f}->{target_el:.1f}"
        )
    if entering_stream:
        rotor_net_cancel_event.set()
    ...
    def handle(self):
        while True:
            ...
            if raw_cmd == "P" or cmd in ("\\set_pos", "set_pos"):
                ...
                if time.time() < float(rotctld_track_control.get("ignoreUntil") or 0.0):
                    with state_lock:
                        latest["status"] = "ROTCTLD SET_POS IGNORED: PARK PRIORITY"
                    self.wfile.write(b"RPRT 0\n")
                    self.wfile.flush()
                    continue
